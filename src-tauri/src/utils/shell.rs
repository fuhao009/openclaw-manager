...
pub fn get_extended_path() -> String {
    ...
    if let Some(home) = dirs::home_dir() {
        ...
        if let Ok(version) = std::fs::read_to_string(&nvm_default) {
            if let Some(version) = normalize_nvm_version(&version) {
                paths.insert(0, nvm_bin_path(&home_str, &version));
            }
        }

        for version in discover_nvm_versions(&home_str) {
            let nvm_bin = nvm_bin_path(&home_str, &version);
...
fn nvm_bin_path(home_str: &str, version: &str) -> String {
    format!("{}/.nvm/versions/node/v{}/bin", home_str, version)
}

fn discover_nvm_versions(home_str: &str) -> Vec<String> {
    let nvm_versions_dir = format!("{}/.nvm/versions/node", home_str);
        ...
        for entry in entries.flatten() {
            if entry.file_type().map(|ft| ft.is_dir()).unwrap_or(false) {
                if let Some(name) = entry.file_name().to_str() {
                    if let Some(v) = normalize_nvm_version(name) {
                        versions.push(v);
                    }
                }
            }
...
fn get_unix_openclaw_paths() -> Vec<String> {
    ...
    if let Some(home) = dirs::home_dir() {
        ...
        if let Ok(version) = std::fs::read_to_string(&nvm_default) {
            if let Some(version) = normalize_nvm_version(&version) {
                paths.insert(0, format!("{}/openclaw", nvm_bin_path(&home_str, &version)));
            }
        }

        for version in discover_nvm_versions(&home_str) {
            paths.push(format!("{}/openclaw", nvm_bin_path(&home_str, &version)));
        }
